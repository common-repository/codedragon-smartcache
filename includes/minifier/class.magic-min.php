...
class Minifier {
    ...
    public $content;
    public $output_file;
    public $output_file_size;
    public $compressed_file_size;
    public $extension;
    private $type;
    ...
    private $config_keys = array(
        ...
        'expiry_offset' => 2678400,     // one month default
        ...
        'include_fonts' => true,        //Expand font paths
        'expand_@imports' => false,     //Expand @import paths (true to return relative paths to absolute; false to remove @imports altogether)
    ...
    private function strip_directory( $input ){
        return str_replace( ABSPATH, '', $input );
    }
    ...
    /**
     * Function to seek out and replace font file references within CSS with a joined list
     *
     * @access private
     * @param string $source_file (used for location)
     * @param string $contents
     * @return string $updated_style
     */
    private function use_source_font_paths( $source_file, $contents ){
        $this->directory = dirname( $source_file ) . DIRECTORY_SEPARATOR;
        $site_url = site_url() . DIRECTORY_SEPARATOR;
        $is_remote = (strpos($source_file, ABSPATH) === false);

        if( preg_match_all( '/url\([\'"]([^"\']+(woff2|woff|eot|ttf|svg)[^"\']+)[\'"]\)/i', $contents, $font_matches, PREG_SET_ORDER ) ){
            $this->find = array();
            $this->replace = array();

            foreach( $font_matches as $font_path ){
                $font_path_arry = preg_split("/[\?\#]+/", $font_path[1]);
                if($is_remote){
                    $real_font_url = $this->directory . $font_path_arry[0];
                }else{
                    $real_font_path = realpath($this->directory . $font_path_arry[0]);
                    $real_font_url  = str_replace(ABSPATH, $site_url, $real_font_path);
                }
                if( (false !== $real_font_path && file_exists( $real_font_path )) || $is_remote ){
                    //Assign the find and replace
                    if(count($font_path_arry) > 1){
                        array_shift($font_path_arry);
                        $real_font_url .= '?' . join('#', $font_path_arry);
                    }
                    $this->find[] = $font_path[0];
                    $this->replace[] = 'url(\'' . $real_font_url . '\')';
                }
            }

            //Log the number of replacements to the console
            self::$messages[]['Minifier Log: merge_fonts'] = count( $this->replace ) .' fonts in ' . $source_file;

            //Find and replace all the fonts paths with real path
            $this->updated_style = str_replace( $this->find, $this->replace, $contents );

            return $this->updated_style;

        }else{
            //No fonts found in the sheet, just return the contents
            return $contents;
        }

    } //end use_source_font_paths()

    /**
     * Function to seek out and replace @import file references within CSS with absolute urls
     *
     * @access private
     * @param string $source_file (used for location)
     * @param string $contents
     * @return string $updated_style
     */
    private function use_absolute_import_paths( $source_file, $contents ){
        $this->directory = dirname( $source_file ) . DIRECTORY_SEPARATOR;
        $site_url = site_url();

        if( preg_match_all( '/@import url\([\'"]([^"\']+)[\'"]\)/i', $contents, $import_matches, PREG_SET_ORDER ) ){
            $this->find = array();
            $this->replace = array();
            $this->prepend = array();

            foreach( $import_matches as $import_path ){
                $real_import_path = realpath($this->directory . $import_path[1]);
                $real_import_url  = str_replace(ABSPATH, $site_url . DIRECTORY_SEPARATOR, $real_import_path);

                if( false !== $real_import_path && file_exists( $real_import_path ) ){
                    //Assign the find and replace
                    $this->find[] = $import_path[0];
                    // $this->replace[] = '/* @IMPORT CSS (' . $real_import_url . ') NOW PREPENDED */';
                    $this->replace[] = '';
                    $this->prepend[] = '@import url(\'' . $real_import_url . '\')';
                }
            }

            //Log the number of replacements to the console
            self::$messages[]['Minifier Log: use_absolute_@imports'] = count( $this->replace ) .' imports in ' . $source_file;

            //Find and replace all the @import paths with real paths
            $this->updated_style = str_replace( $this->find, $this->replace, $contents );

            return array('prepend' => join(";" . PHP_EOL, $this->prepend) . ";", 'style' => $this->updated_style);

        }else{
            //No @import paths found in the sheet, just return the contents
            return array('prepend' => null, 'style' => $contents);
        }

    } //end use_absolute_import_paths()

    /**
     * Function to seek out and delete @import file references within CSS
     *
     * @access private
     * @param string $source_file (used for location)
     * @param string $contents
     * @return string $updated_style
     */
    private function remove_import_paths( $source_file, $contents ){
        $this->directory = dirname( $source_file ) . DIRECTORY_SEPARATOR;
        $site_url = site_url();
        ...
        if( preg_match_all( '/@import url\([\'"]([^"\']+)[\'"]\);/i', $contents, $import_matches, PREG_SET_ORDER ) ){
            ...
            foreach( $import_matches as $import_path ){
                ...
                if( false !== $real_import_path && file_exists( $real_import_path ) ){
                    //Assign the find and replace
                    $this->find[] = $import_path[0];
                    $this->replace[] = '/* @IMPORT CSS (' . $real_import_url . ') REMOVED */';
                }
            }

            //Log the number of replacements to the console
            self::$messages[]['Minifier Log: remove_@imports'] = count( $this->replace ) .' imports in ' . $source_file;

            //Find and remove all the @import paths
            $this->updated_style = str_replace( $this->find, $this->replace, $contents );
    ...
    private function minify_contents( $src_file, $run_minification = true ){
        ...
        if( preg_match( '/\.min\./i', $src_file ) || $run_minification === false ){
            ...
            if( !empty( $this->type ) && $this->type == 'css' ){
                ...
                // Smart-cache modification: force use of source font and @import file paths
                if($this->settings['include_fonts']){
                    $this->content = $this->use_source_font_paths( $src_file, $this->content );
                }

                if($this->settings['expand_@imports']){
                    $data = $this->use_absolute_import_paths( $src_file, $this->content );
                    $prepend = $data['prepend'];        // split the @import line from the rest of the content
                    $this->content = $data['style'];
                }else{
                    $this->content = $this->remove_import_paths( $src_file, $this->content );
                }
            }
            return array('prepend' => $prepend, 'content' => $this->content);
        }else{
            if( !empty( $this->type ) && $this->type == 'css' ){
                $this->content = $this->source;
                ...
                //Rmove tabs, spaces, newlines, etc.
            ...
            if( !empty( $this->type ) && $this->type == 'js' ){
                ...
                if( $this->settings['closure'] ){
                    ...
                    if( isset( $result_raw['serverErrors'] ) && !empty( $result_raw['serverErrors'] ) ){
                        ...
                        echo $e_code . ': '. $e_message . ' File: '. basename( $src_file );
                    }else{
                ...
                // Smart-cache modification: append semi-colon after jQuery closure
                if(substr($this->content, -1, 1) !== ';') $this->content .= ';';

            } //end $this->type == 'js'

            //Add to the output and return it
            $this->output .= $this->content;
            return array('prepend' => $prepend, 'content' => $this->output);
        }
     ...
     * @return array(new filename/location (same as path to variable), file size)
     */
    private function make_min( $src_file, $new_file, $do_minify = true ){
        ...
        if( $this->settings['gzip'] ){
            ...
            // $this->prequel .= 'date_default_timezone_set(\'America/Los_Angeles\');' . PHP_EOL;
            ...
            $offset = intval($this->settings['expiry_offset']);
        ...
        if( !is_array( $src_file ) ){
            //Single file
            //
            $minified_data = $this->minify_contents( $src_file, $do_minify );
            $file_size = $this->get_file_size($minified_data['content']);

            $this->filetag = '/**' . PHP_EOL;
            $this->filetag .= ' * Generated by ' . SMART_CACHE_PLUGIN_NAME_PROPER . ': ' . date('Y-m-d h:i:s A') . PHP_EOL;
            $this->filetag .= ' * - Filename: ' . $src_file . PHP_EOL;
            $this->filetag .= ' */' . PHP_EOL;

            // $this->content = trim( $this->prequel . $this->filetag . $minified_data['prepend'] . $minified_data['content'] );
            $this->content = trim( $this->prequel . $minified_data['prepend'] . $minified_data['content'] );
            $file_contents_ok = true;
        }else{
            //Multiple files
            //
            ...
            $this->filetag = '/**' . PHP_EOL;
            $this->filetag .= ' * Generated by ' . SMART_CACHE_PLUGIN_NAME_PROPER . ': ' . date( 'Y-m-d h:i:s A' ). PHP_EOL;
            $this->filetag .= ' * Table of contents: ' . PHP_EOL;
            $this->filetag .= ' * - ' . implode( PHP_EOL .  ' * - ', $filenames ) . PHP_EOL;
            $this->filetag .= ' */' . PHP_EOL;
            ...
            foreach( $src_file as $this->new_file ){
                ...
                $minified_data = $this->minify_contents( $this->new_file, $do_minify );
                if(!empty($minified_data)){
                    $file_head = '/* File: ' . $this->new_file . (($do_minify) ? ' - minified' : ' - raw') . ' */';
                    $file_head = null;

                    // Smart-cache modification: sum file sizes
                    $file_size += $this->get_file_size($minified_data['content']);
                    // $minified_data['content'] .= PHP_EOL;

                    $prepended .= $minified_data['prepend'];        // join all the prepends together
                    $this->compiled .= $file_head . trim($minified_data['content']) . PHP_EOL;
                }
                $file_contents_ok = true;
            }

            //Write the temporary contents to the full contents
            // $this->content = trim( $this->prequel . $this->filetag . $prepended . $this->compiled );
            $this->content = trim( $this->prequel . $prepended . trim($this->compiled ) );
        ...
        if($file_contents_ok){
            ...
            if( !$this->handle ){
                ...
                return array($new_file, $file_size);
            }
        }else{
            //Nothing to return
            return array(false, 0);
     ...
     * @return array array($output_file (includes provided location), file size)
     */
    public function minify( $src_file, $file = '', $version = '' ){
        ...
        if( $this->remote_file( $src_file ) && file_exists( $file ) ){
            ...
            list($this->output_file, $this->output_file_size) = $this->make_min( $src_file, $file, false );
            ...
            list($this->output_file, $this->output_file_size) = $this->make_min( $src_file, $file );
        ...
        if( $this->settings['echo'] ){
            ...
            return array($this->output_file, $this->output_file_size);
        }
     ...
     * @return array(new filenae, file_size)
     */
    public function merge( $output_filename, $directory, $list_or_type = 'js', $exclude = array(), $order = array() ){
        ...
        if( !empty( $order ) ){
            ...
            $this->directory = array_diff( $this->compilation, $this->directory );
        ...
        if( file_exists( $minified_name ) && $this->create_new ){
            ...
            $this->compilation = array_unique($this->compilation);
            list($this->compressed, $this->compressed_file_size) = $this->make_min( $this->compilation, $output_filename, true );
        ...
        if( $this->settings['echo'] ){
            ...
            return array($this->compressed, $this->compressed_file_size);
        }

    } //end merge()

    /**
     * Return the size of the string contents after removing embedded binary data
     * @param  string $content
     * @return integer
     */
    public function get_file_size($content){
        $content = preg_replace('/(url\(data:.*\))/', '', $content);
